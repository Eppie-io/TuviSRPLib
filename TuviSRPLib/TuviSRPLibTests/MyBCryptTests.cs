...
//        [Test]
//        public void DoTestShortKeys()
//        {
//            byte[] salt = new byte[16];

//            // Check BCrypt with empty key pads to zero byte key
//            byte[] hashEmpty = MyBCrypt.Generate(new byte[0], salt, 4);
//            byte[] hashZero1 = MyBCrypt.Generate(new byte[1], salt, 4);

//            Assert.That(hashZero1, Is.EqualTo(hashEmpty));

//            // Check zeroed byte key of min Blowfish length is equivalent
//            byte[] hashZero4 = MyBCrypt.Generate(new byte[4], salt, 4);
//            Assert.That(hashZero4, Is.EqualTo(hashEmpty));

//            // Check BCrypt isn't padding too small (32 bit) keys
//            byte[] hashA = MyBCrypt.Generate(new byte[] { (byte)'a' }, salt, 4);
//            byte[] hashA0 = MyBCrypt.Generate(new byte[] { (byte)'a', (byte)0 }, salt, 4);
//            Assert.AreNotEqual(hashA, hashA0);            
//        }
...
//                byte[] hash = MyBCrypt.Generate(password, salt, cost);
//                Assert.That(hash, Is.EqualTo(expected));
//            }

//            Assert.That(Hex.Decode("cdd19088721c50e5cb49a7b743d93b5a6e67bef0f700cd78"), Is.EqualTo(BCrypt.Generate(BCrypt.PasswordToByteArray("12341234".ToCharArray()), Hex.Decode("01020304050607080102030405060708"), 5)));
//            Assert.That(Hex.Decode("02a3269aca2732484057b40c614204814cbfc2becd8e093e"), Is.EqualTo(BCrypt.Generate(BCrypt.PasswordToByteArray("1234".ToCharArray()), Hex.Decode("01020304050607080102030405060708"), 5)));
//        }
